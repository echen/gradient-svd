...
class Svd
  ...
  def self.lsi(documents)    
    ...
    documents.each_with_index do |document, i|
      rows[i] = {}
      document.split.uniq.each do |word|
...
=end
  ...
  def compute(options = {})    
    options = {:num_features => 2,
              ...
              :annealing_rate => 500,
              :regularization => 0,
              :min_improvement => 0.0001}.merge(options)
                
    0.upto(options[:num_features] - 1) do |feature|
      @log << "# Feature #{feature}"
      
      # Initialize the new feature.
      (@row_vectors + @col_vectors).each{ |vector| vector << 0.1 } # rand }
      ...
      0.upto(options[:max_epochs] - 1) do |epoch|
        ...
        @rows.each_with_index do |row, i|
          row.each_pair do |col, value|
            ...
            @row_vectors[i][feature] += options[:initial_learning_rate] * (error * col_feature - options[:regularization] * row_feature)
            @col_vectors[col][feature] += options[:initial_learning_rate] * (error * row_feature - options[:regularization] * col_feature)
          end
        end
